...
METRIC_SECTIONS = ("native_clean", "care_task1", "supplemental")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate CLEAN Leak-CURBER seed metrics")
...
def iter_metric_files(runs_root: str | Path) -> list[Path]:
    root = Path(runs_root)
    return sorted(root.glob("*/seeds/*/results/*_metrics.json"))
...
def metric_context(metric_path: Path) -> tuple[str, str, str]:
    seed = metric_path.parents[1].name
    split_group = metric_path.parents[3].name
    eval_split = metric_path.name.removesuffix("_metrics.json")
...
def write_aggregate_csvs(
    ...
    *,
    ...
    summary_df = (
        long_df.groupby(["split_group", "eval_split", "metric"])["value"]
