...
It is designed for Leak-CURBER-style train/val/test split trees under:

- `<PROJECT_ROOT>/data/processed/baselines/EITLEM-Kinetics`
...
- The bench reuses the original repo modules from [Code/KCM.py](../Code/KCM.py), [Code/KMP.py](../Code/KMP.py), and [Code/ensemble.py](../Code/ensemble.py)
...
- The default base directory assumes the Leak-CURBER baseline folder is named `EITLEM-Kinetics`. If your folder name differs, pass `--base_dir` explicitly.
