...
PROJECT_DATA_ROOT = Path(os.environ.get("PROJECT_ROOT", ".")) / "data" / "processed" / "baselines"
...
def main():
    ...
    for target in TARGETS:
        target_root = PROJECT_DATA_ROOT / target
        retrain_root = target_root / "retrain_from_optuna"
        ...
        for seed_dir in sorted(retrain_root.rglob("seed_*")):
            ...
            if split_results is None:
                print(f"  [skip] incomplete: {seed_dir.relative_to(PROJECT_DATA_ROOT)}")
                continue
    ...
    for target in TARGETS:
        ...
        out_path = PROJECT_DATA_ROOT / target / "aggregate_eitlem_results.csv"
