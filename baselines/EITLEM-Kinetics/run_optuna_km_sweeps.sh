...
python emulator_bench/launch_parallel_optuna.py \
  --gpus 0 1 \
  --trials_per_gpu 3 \
  --base_dir ${PROJECT_ROOT}/data/processed/baselines/EITLEM-Kinetics \
  --embeddings_dir ${PROJECT_ROOT}/data/processed/baselines/EITLEM-Kinetics/embeddings \
  ...
  --pin_memory \
  --batch_size 256 \
  --storage sqlite:///${PROJECT_ROOT}/data/processed/baselines/EITLEM-Kinetics/optuna_studies/eitlem_km_enz_0.09.db \
  2>&1 | tee "$enz_log"
...
python emulator_bench/launch_parallel_optuna.py \
  ...
  --pin_memory \
  --batch_size 256 \
  --storage sqlite:///${PROJECT_ROOT}/data/processed/baselines/EITLEM-Kinetics/optuna_studies/eitlem_km_sub_0.45.db \
  2>&1 | tee "$sub_log"
