...
class GNN(torch.nn.Module): # from Yang
        ...
        def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, gnn_type="gin", pretrained = False):
        ...
        def forward(self, *argv):
            ...
            if(torch.any(torch.isnan(node_representation))):
                print('torch.any(torch.isnan(self.x_embedding1.weight))', torch.any(torch.isnan(self.x_embedding1.weight)))
                print('torch.any(torch.isnan(self.x_embedding2.weight))', torch.any(torch.isnan(self.x_embedding2.weight)))
                print('torch.any(torch.isnan(self.x_embedding3.weight))', torch.any(torch.isnan(self.x_embedding3.weight)))
                print('torch.any(torch.isnan(self.x_embedding4.weight))', torch.any(torch.isnan(self.x_embedding4.weight)))
                print('torch.any(torch.isnan(self.x_embedding5.weight))', torch.any(torch.isnan(self.x_embedding5.weight)))
                print('torch.any(torch.isnan(self.x_embedding6.weight))', torch.any(torch.isnan(self.x_embedding6.weight)))
